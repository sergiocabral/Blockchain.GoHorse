...
export class ExtractFieldForKeyInfo {
  ...
  public static mainKeyExpires(output: string): Date | undefined {
    const regexToExtract =
      /pub\s+.*(?<=expires: )(?<year>\d{4})-(?<month>\d{2})-(?<day>\d{2})/;
  ...
  /**
   * Nome do algoritmo da sub chave.
   */
  public static subKeyAlgorithm(output: string): string | undefined {
    const regexToExtract = /sub\s+(\w+[a-zA-Z])\d+/;
    const extracted = regexToExtract.exec(output);
   ...
   */
  public static subKeyLength(output: string): number | undefined {
    const regexToExtract = /sub\s+\w+[a-zA-Z](\d+)\W/;
    const extracted = regexToExtract.exec(output);
   ...
   */
  public static subKeyExpires(output: string): Date | undefined {
    const regexToExtract =
      /sub\s+.*(?<=expires: )(?<year>\d{4})-(?<month>\d{2})-(?<day>\d{2})/;
